# 👋 Hi, I'm Mpho Matheba

**Bachelor of Computer and Information Sciences in Application and Development**

Full Stack Developer | C# & .NET Enthusiast | Problem Solver

---

## 🎮 Watch the Snake Eat My Contributions!

<img 
  alt="GitHub Snake" 
  src="https://raw.githubusercontent.com/tobiasmeyhoefer/tobiasmeyhoefer/output/github-snake-dark.svg" 
  height="200" 
  style="background-color: transparent;" />

---

## 💼 What I Do

- 🔨 Build web applications with **C#** and **.NET Core**
- 💾 Design databases with **SQL** and **Entity Framework**
- 🎨 Create responsive UIs with **Bootstrap** and **JavaScript**
- ☁️ Deploy to **Azure Cloud**

---

## 🚀 Featured Project

### EventEase - Event Management System
**[View on GitHub](https://github.com/EMKNDN/cldv6211-g1-2026-poe-MphoMatheba)**

A complete event booking platform with:
- Event & venue management
- Advanced search & filtering
- Booking confirmation system
- Responsive design

**Stack:** ASP.NET Core | SQLite | Bootstrap

---

## 🛠️ Tech Stack

```
Languages:    C#, JavaScript, SQL, HTML/CSS
Frameworks:   ASP.NET Core, Entity Framework Core
Databases:    SQL Server, SQLite
Cloud:        Azure (Blob Storage)
Tools:        Git, GitHub, Visual Studio, VS Code
```

---

## 📜 Licenses & Certifications

- **Introduction to Web Development** - SheCodes (Apr 2025)
  - Skills: HTML, CSS, JavaScript, Responsive Design

- **Introduction to Coding** - SheCodes (Apr 2025)
  - Skills: HTML, CSS, JavaScript, Web Basics

---

## 🎓 Education

**Bachelor of Computer and Information Sciences in Application and Development**

---

## 📬 Connect With Me

📧 **Email:** mphomatheba047@gmail.com  
🐙 **GitHub:** [@MphoMatheba](https://github.com/MphoMatheba)  
💼 **LinkedIn:** [Mpho Matheba](https://www.linkedin.com/in/mpho-matheba-b16a53293/)

---

<div align="center">

![Profile Views](https://komarev.com/ghpvc/?username=MphoMatheba&color=brightgreen&style=flat)

**⭐ Star my repos if you like what you see!**

</div>
